...
def heatmap(data, row_labels=None, col_labels=None, ax=None, cbar=True,
    ...
    **kwargs
    ...
    elif row_labels and not col_labels:
        ax.set_yticks(np.arange(data.shape[0]))
        ax.set_yticklabels(row_labels, fontsize=16)
...
def annotate_heatmap(im, data=None, valfmt="{x:.2f}",
                     ...
                     threshold=None, threshold2 = None, ignore_diagonal=False, **textkw):
    """
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            if i == j and ignore_diagonal:
                continue
            kw.update(color=textcolors[int(data[i, j] >= threshold and data[i, j] <= threshold2)])
