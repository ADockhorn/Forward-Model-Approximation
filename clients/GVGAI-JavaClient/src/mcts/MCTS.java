...
public class MCTS {
	...
	public Move runMCTS_UCT(Board startingBoard, int runs, boolean bounds) {
		...
		if (!pmode) {
            ...
            Node c = maxChild(rootNode); // Select best performing child
            bestMoveFound = c.move;

            if (MCTSPARAMETERS.MCTS_DEBUG){
                Node mC = minChild(rootNode);
                if (c.score[0] - mC.score[0] > 0){
                    System.out.println("DEBUG: Observed Score Difference: " + String.valueOf(c.score[0] - mC.score[0]));
                }
            }
    ...
    /**
     * Select the child node with the lowest score (only for debugging)
     *
     * @param n
     * @return
     */
    private Node minChild(Node n) {
        double worstValue = Double.POSITIVE_INFINITY;
        double tempWorst;
        Node worstNode = null;

        for (Node s : n.children) {
            tempWorst = s.score[n.player];
            tempWorst += s.opti[n.player] * optimisticBias;
            tempWorst += s.pess[n.player] * pessimisticBias;
            if (tempWorst < worstValue) {
                worstNode = s;
                worstValue = tempWorst;
            }
        }
        return worstNode;
    }
